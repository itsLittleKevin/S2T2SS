...
from gpu_stream_manager import GPUStreamManager
...
class S2T2SSPipeline:
    ...
    def __init__(self):
        ...
        # Initialize GPU stream manager for concurrent GPU operations
        if config.ENABLE_GPU_STREAMS and config.ENABLE_GPU_ASR:
            self.gpu_stream_manager = GPUStreamManager()
            print("GPU Stream Manager initialized for concurrent ASR+TTS operations")
        else:
            self.gpu_stream_manager = None
        ...
        if config.OPTIMIZED_CHUNK_SIZE:
            ...
            self.chunk_duration = 5.0               # Extended for maximum context and accuracy
            self.overlap_duration = 0.3             # Increased overlap for better accuracy
    ...
    def process_audio_chunk(self, audio_chunk: bytes, start_time: float = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # Use GPU stream manager for ASR if available
            if self.gpu_stream_manager and config.ENABLE_GPU_ASR:
                raw_text = self.gpu_stream_manager.process_asr(self.asr, audio_chunk)
            else:
                raw_text = self.asr.transcribe_chunk(audio_chunk)
                
            # Replace all '呃' with '嗯' in ASR output
            raw_text = raw_text.replace('呃', '嗯')
            ...
            if not raw_text:
                try:
                    ...
                    print(f"⏱️ ASR Time: {asr_time:.3f}s (silent)")
                except UnicodeEncodeError:
                    print("No speech detected in chunk")
                    print(f"ASR Time: {asr_time:.3f}s (silent)")
                return results
            
            results['raw_text'] = raw_text
            try:
                print(f"🎤 ASR Result: '{raw_text}'")
                print(f"⏱️ ASR Time: {asr_time:.3f}s")
            except UnicodeEncodeError:
                print(f"ASR Result: '{raw_text}'")
            ...
            if config.ENABLE_TTS:
                ...
                # Use GPU stream manager for TTS if available
                if self.gpu_stream_manager:
                    audio_generated = self.gpu_stream_manager.process_tts(
                        self.tts, processed_text, save_file=True, play_audio=True
                    )
                else:
            ...
            try:
                print(f"⏱️ Caption Time: {caption_time:.3f}s")
            except UnicodeEncodeError:
                print(f"Caption Time: {caption_time:.3f}s")
    ...
    def stop_live_processing(self):
        ...
        # Clean up GPU stream manager
        if self.gpu_stream_manager:
            self.gpu_stream_manager.cleanup()
            print("✅ GPU streams cleaned up")
