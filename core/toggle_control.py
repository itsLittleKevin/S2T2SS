...
def show_current_configuration():
    ...
    print("Current S2T2SS Configuration")
    print("=" * 50)
    ...
    print(f"Chunk Duration:       {'3 seconds (Speed)' if config.OPTIMIZED_CHUNK_SIZE else '5 seconds (Accuracy)'}")
    print(f"ASR Processing:       {'GPU Mode' if config.ENABLE_GPU_ASR else 'CPU Mode'}")
    print(f"Narrator Mode:        {config.NARRATOR_MODE.upper()}")
    print(f"Output Language:      {config.OUTPUT_LANGUAGE.upper()}")
    print(f"TTS Voice Style:      {config.TTS_VOICE.upper()}")
...
def toggle_chunk_duration():
    """Toggle between 3-second and 5-second audio chunks"""
    config.OPTIMIZED_CHUNK_SIZE = not config.OPTIMIZED_CHUNK_SIZE
    if config.OPTIMIZED_CHUNK_SIZE:
        duration = "3 seconds (Speed Mode)"
        print("Chunk Duration: 3 seconds (faster processing)")
    else:
        duration = "5 seconds (Accuracy Mode)"
        print("Chunk Duration: 5 seconds (better accuracy)")
    
    # Show the impact
    print(f"   Current setting optimized for {'speed' if config.OPTIMIZED_CHUNK_SIZE else 'accuracy'}")

def toggle_gpu_asr():
    """Toggle between GPU and CPU ASR processing"""
    config.ENABLE_GPU_ASR = not config.ENABLE_GPU_ASR
    if config.ENABLE_GPU_ASR:
        print("ASR Processing: GPU Mode (faster processing)")
        print("   Note: Requires ASR engine restart to take effect")
    else:
        print("ASR Processing: CPU Mode (stable, lower memory)")
        print("   Note: Requires ASR engine restart to take effect")
    ...
    try:
        ...
        if torch.cuda.is_available():
            ...
            print(f"   Available GPU: {gpu_name} ({gpu_memory:.1f}GB VRAM)")
        else:
...
def interactive_configuration_menu():
    ...
    while True:
        ...
        print("\n🎮 Toggle Options:")
        print("1.  Toggle LLM Text Editing")
        print("2.  Toggle OBS Caption Output") 
        print("3.  Toggle TTS Audio Output")
        print("4.  Toggle Chunk Duration (3s/5s)")
        print("5.  Toggle ASR Processing (CPU/GPU)")
        
        print("\n🎯 Language & Voice Settings:")
        print("6.  Cycle Narrator Mode")
        print("7.  Cycle Output Language")
        print("8.  Cycle TTS Voice Style")
        
        print("\n🚀 Performance Modes:")
        print("9.  Speed Mode (Fastest)")
        print("10. Text Mode (Silent)")
        print("11. Quality Mode (Full)")
        print("12. Podcast Mode (Natural)")
        
        print("\n📋 Options:")
        print("13. Save Configuration")
        print("14. Test Toggle Logic")
        print("15. Cache Management")
        print("0.  Exit")
        
        try:
            choice = input("\nYour choice (0-15): ").strip()
...
def cache_management_menu():
    ...
    while True:
        ...
        print(f"\n🎛️ Cache Options:")
        print("1.  Toggle Auto Cleanup")
        print("2.  Clean Old Files")
        print("3.  Clear All Cache")
        print("4.  Configure Max Files")
        print("5.  Configure Max Age")
