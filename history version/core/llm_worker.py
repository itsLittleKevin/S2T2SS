...
class LLMWorker:
    ...
    def process_text(self, text: str, mode: str = "refine") -> str:
        ...
        # Fast processing mode: Use segmentation only for speed (unless translation needed)
        if hasattr(config, 'FAST_PROCESSING_MODE') and config.FAST_PROCESSING_MODE and not needs_translation:
...
只返回改进后的中文文本，不要解释。"""
                ...
                return f"""Please improve this speech-to-text output by:
1. Correcting obvious transcription errors
2. Adding proper punctuation
3. Fixing grammar while preserving the original meaning
4. IMPORTANT: Keep the SAME LANGUAGE as the input (do not translate)
5. Maintain the original tone and style

Text to improve: "{text}"

Return only the improved text in the SAME LANGUAGE, no explanations."""

        elif mode == "correct":
            if is_chinese:
                return f"""请纠正这段语音转文字的输出：
- 修正拼写错误
- 添加缺失的标点符号
- 保持原意不变
- 保持中文输出
...
只返回纠正后的中文文本。"""
                ...
                return f"""Please correct this speech-to-text output:
- Fix spelling errors
...
Return only the enhanced text in the SAME LANGUAGE."""
    ...
    def _clean_text_for_tts(self, text: str) -> str:
        ...
        # Remove excessive dots/dashes that cause pauses
        cleaned = re.sub(r'\.{3,}', '...', cleaned)  # Max 3 dots
        ...
        # Ensure sentences end properly (helps TTS timing)
